...
let input = document.querySelector(".container .search-box input");
...
let getPokemonTypes = (types) => {
    ...
    types.forEach((typ) =>{
        let span = document.createElement("span") // criando span para cada tipo do pokemon escolhido
...
let styleCard = (color) => {
    ...
    pokeTypeBox.querySelectorAll("span").forEach(
        (typeColor) => typeColor.style.background = color
    );
}
